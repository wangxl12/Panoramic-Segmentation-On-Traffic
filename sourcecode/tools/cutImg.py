...
class CutImage():
    ...
    def __init__(self, boxes=None, scores=None, labels=None, metadata=None, panoptic_seg=None, segments_info=None,
                 image=None, isThing=True, isNotThing=True, box_labels=None):
        self.image = image
        self.boxes = boxes
        self.scores = scores
        self.labels = labels
    ...
    def cutIsThing(self):
        ...
        for id, box in enumerate(self.boxes):
            x0, y0, x1, y1 = box
            # if self.labels[id] != self.box_labels[id]:
            #     continue
...
class _PanopticPrediction:
    def __init__(self, panoptic_seg, segments_info):
        ...
        # sorted_idxs = np.array(range(1, len(self._sinfo) + 1))
        # print(self._sinfo)
        self._seg_ids, self._seg_areas = segment_ids, areas
